...
Version: 1.6
Author: DaddyDesign
Tags: footer, toolbar, social networking, social icons, tool bar, share, facebook like, tweet, recent tweet, facebook, twitter, settings, customize, colors,wibiya, social toolbar,google +1,google plusone,plusone,google share,pinit,pinterest,pin it button,pin it bar
...
function DDST_admin_scripts()
{
	...
	if(isset($_GET['page'])&&($_GET['page']=='social_toolbar_profiles'||$_GET['page']=='social-toolbar/social-toolbar.php')){	
...
function DDST_new_tweets()
{
	...
	if (false === ( $fs_tweets = get_transient('DDST_socialtoolbar_1_5') )|| $social_icons[1]['url']!=get_transient('DDST_pro_tweets_user') ) {//if tweets are not in the cache
     $fs_tweets = DDST_get_tweets();//fetch them
     set_transient('DDST_socialtoolbar_1_5', $fs_tweets, 60*60);//cache them for 1 hour
...
{	
	...
	else
	{
		if(is_singular())
		{
			global $post,$posts;
			$page_id=$post->ID;
			if(isset($social_options['blog_single_post']))
			{
		...
		if(is_front_page() && isset($social_options['home_page']))
		{
			DDST_html_code_footer();
		}
		if(is_archive() && isset($social_options['category_archive']))
		{
			DDST_html_code_footer();
		}
	}
		...
		<div id="basic-modal-content">
			<div id="wpsocial-toolbar-go-pro">
				<div class="wpsocial-toolbar-go-pro-left">
					...
					<h2><?php _e('WHY GO PRO? ','WPSOCIALTOOLBAR'); ?><span><a href="http://socialtoolbarpro.com" title="Wp Social Toolbar Pro" target="_blank">CLICK HERE TO FIND OUT</a></h2>
				</div>
