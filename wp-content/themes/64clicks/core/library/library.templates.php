...
function pagelines_draw_sidebar($id, $name){?>
	<ul id="<?php echo 'list_'.$id; ?>" class="sidebar_widgets fix">
		<?php if (!dynamic_sidebar($name)){ pagelines_default_widget($id, $name); } ?>
	</ul>
<?php }
 ...
 *
 */
function pagelines_default_widget($id, $name){
	if(!pagelines('sidebar_no_default')):
	?>	
		...
		<div class="current_posts_info">
			...
			<?php elseif(is_archive()):?>
				<?php if (is_author()) { ?>
					...
					<strong><?php echo get_author_name(get_query_var('author')); ?></strong>
				<?php } elseif (is_day()) {	?>
